      ...
      >
        {categoryData.map((item) => (
          <div
            ...
            className={`w-40 h-40 flex-shrink-0  flex justify-center items-center border-[1px] p-5  cursor-pointer transition-all duration-700 ease-in-out delay-100 ${
              acticeId === item.id
                ? "bg-accent text-secondaryText"
                : "hover:bg-accent hover:text-secondaryText"
            }`}
          >
            <h3 className="text-sm sm:text-lg lg:text-xl">
              {item.categoryName}
            </h3>
          </div>
        ))}
