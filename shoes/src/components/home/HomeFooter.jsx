  ...
  return (
    <section className="w-full   overflow-hidden bg-gray-300">
        <div className="  p-5 flex flex-wrap justify-evenly items-center  gap-2">
            {
                data.map((item)=>(
                    <div key={item.id} className="h-[600px] w-[600px] flex flex-col justify-center items-center bg-cover bg-no-repeat transition duration-700 delay-100 ease-in-out transform hover:scale-[1.01] rounded-4xl hover:cursor-pointer" style={{backgroundImage:`url(${item.img})`}}>
                        <h1 className="font-bold sm:text-2xl lg:text-4xl text-gray-300 ">{item.name}</h1>
                        <p className="sm:text-lg lg:text-xl font-thin text-center">{item.description}</p>
